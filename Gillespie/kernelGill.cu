...
float* output;
float* dev_output;
output = (float*)malloc(223*69*sizeof(float));
for(int i = 0; i < 223*69; i++){
output[i] = 0;
}
cudaStatus = cudaMalloc(&dev_output, 223*69*sizeof(float));
if (cudaStatus != cudaSuccess) {fprintf(stderr, "cudaMalloc failed!");goto Error;}
cudaStatus = cudaMemcpy(dev_output, output, 223*69*sizeof(float), cudaMemcpyHostToDevice);
...
simulate<<<1, 32>>>(i, dev_output, devStates, 900.0000000000, 200000.0000000000, 10000, species_global);cudaStatus = cudaGetLastError(); if (cudaStatus != cudaSuccess) {fprintf(stderr, "addKernel launch failed: %s\n", cudaGetErrorString(cudaStatus));goto Error;}
...
cudaStatus = cudaMemcpy(output, dev_output, 223*69*sizeof(float), cudaMemcpyDeviceToHost);
if (cudaStatus != cudaSuccess) {fprintf(stderr, "cudaMemcpy failed!");goto Error;}
